...
t[#t+1]=Def.ActorFrame{
	...
	LoadActor(THEME:GetPathB("","_door/cosh_choice_difficulty_base.png"))..{
		InitCommand=function(self)
			self:x(SCREEN_LEFT-OffsetX-28):y(SCREEN_CENTER_Y+160):zoom(1);
		end;
		OnCommand=function(self)
			self:sleep(SleepOffset+0.2):linear(0.05):x(SCREEN_LEFT+OffsetX+3);
		end;
	};
	
	LoadActor(THEME:GetPathB("","_door/cosh_choice_best_score.png"))..{
		InitCommand=function(self)
			self:x(SCREEN_LEFT-OffsetX):y(SCREEN_CENTER_Y+206):zoom(1);
		end;
	OnCommand=function(self)
		self:sleep(SleepOffset+0.2):linear(0.05):x(SCREEN_LEFT+OffsetX);
	end;
	};
	...
	InitCommand=cmd(player,PLAYER_1;zoom,0.27;shadowlength,1;x,SCREEN_LEFT+167-OffsetX;y,SCREEN_CENTER_Y+198;horizalign,center;draworder,2;);
	OnCommand=function(self)
			local SongOrCourse, StepsOrTrail;
			if GAMESTATE:IsCourseMode() then
				SongOrCourse = GAMESTATE:GetCurrentCourse();
...
--Grade	
	Def.Quad{
	InitCommand=cmd(player,PLAYER_1;zoom,0.95;shadowlength,1;x,SCREEN_LEFT+140-OffsetX;y,SCREEN_CENTER_Y+196;horizalign,center;draworder,2;);
		...
		OnCommand=function(self)
			...
			(cmd(sleep,SleepOffset+0.2;linear,0.05;x,SCREEN_LEFT+OffsetX+140))(self);
		end;
	};
	...
	Def.RollingNumbers { -- Topscore
			...
			InitCommand=cmd(shadowlength,0;zoomy,0.80;zoomx,1.85;maxwidth,240;x,SCREEN_LEFT-OffsetX-40;y,SCREEN_CENTER_Y+200;horizalign,center;);
			OnCommand=function(self)
				if GAMESTATE:IsCourseMode() then
...
LoadFont("_helveticaneuelt pro 65 md 24px") .. {
	...
	InitCommand=cmd(maxwidth,200;zoomy,0.8;zoomx,1.4;x,SCREEN_LEFT-OffsetX-170;y,SCREEN_CENTER_Y+230;horizalign,left;strokecolor,Color("Outline"));
	OnCommand=function(self)
		(cmd(sleep,SleepOffset+0.2;linear,0.05;x,SCREEN_LEFT+OffsetX-170))(self);
	end;
};
--Texture=THEME:GetPathG("","_shared/folder lamp base.png"),
LoadActor(THEME:GetPathB("","Region"))..{
	InitCommand=cmd(x,SCREEN_LEFT-OffsetX+105;y,SCREEN_CENTER_Y+230;zoom,1);
		OnCommand=cmd(sleep,SleepOffset+0.2;linear,0.05;x,SCREEN_LEFT+OffsetX+105);
...
t[#t+1]=Def.ActorFrame{
	...
	LoadActor(THEME:GetPathB("","_door/cosh_choice_difficulty_base.png"))..{
		InitCommand=function(self)
			self:x(SCREEN_RIGHT+OffsetX+28):y(SCREEN_CENTER_Y+160):zoomx(-1):zoomy(1);
		end;
		OnCommand=function(self)
			self:sleep(SleepOffset+0.2):linear(0.05):x(SCREEN_RIGHT-OffsetX-3);
		end;
	};
	
	LoadActor(THEME:GetPathB("","_door/cosh_choice_best_score.png"))..{
		InitCommand=function(self)
			self:x(SCREEN_RIGHT+OffsetX):y(SCREEN_CENTER_Y+206):zoom(1);
		end;
	OnCommand=function(self)
		self:sleep(SleepOffset+0.2):linear(0.05):x(SCREEN_RIGHT-OffsetX);
	end;
	};
	--FC Ring
	LoadActor(THEME:GetPathG("StageIn","Spin FullCombo"))..{
	InitCommand=cmd(player,PLAYER_2;zoom,0.27;shadowlength,1;x,SCREEN_RIGHT+167+OffsetX;y,SCREEN_CENTER_Y+198;horizalign,center;draworder,2;);
	OnCommand=function(self)
			local SongOrCourse, StepsOrTrail;
			if GAMESTATE:IsCourseMode() then
				SongOrCourse = GAMESTATE:GetCurrentCourse();
	...
	Def.Quad{
	InitCommand=cmd(player,PLAYER_2;zoom,0.95;shadowlength,1;x,SCREEN_RIGHT+140+OffsetX;y,SCREEN_CENTER_Y+196;horizalign,center;draworder,2;);
		...
		OnCommand=function(self)
			...
			(cmd(sleep,SleepOffset+0.2;linear,0.05;x,SCREEN_RIGHT-OffsetX+140))(self);
		end;
	};
	...
	Def.RollingNumbers { -- Topscore
			File = THEME:GetPathF("_helveticaneuelt pro 65 md","24px");
			InitCommand=cmd(shadowlength,0;zoomy,0.8;zoomx,1.85;maxwidth,240;x,SCREEN_RIGHT+OffsetX-40;y,SCREEN_CENTER_Y+200;horizalign,center;);
			OnCommand=function(self)
				if GAMESTATE:IsCourseMode() then
					self:Load("RollingNumbersCourseData");
				else
...
LoadFont("_helveticaneuelt pro 65 md 24px") .. {
	...
	InitCommand=cmd(maxwidth,200;zoomy,0.8;zoomx,1.4;x,SCREEN_RIGHT+OffsetX-170;y,SCREEN_CENTER_Y+230;horizalign,left;strokecolor,Color("Outline"));
	OnCommand=function(self)
		(cmd(sleep,SleepOffset+0.2;linear,0.05;x,SCREEN_RIGHT-OffsetX-170))(self);
	end;
};
--Texture=THEME:GetPathG("","_shared/folder lamp base.png"),
LoadActor(THEME:GetPathB("","Region"))..{
	InitCommand=cmd(x,SCREEN_RIGHT+OffsetX+105;y,SCREEN_CENTER_Y+230;zoom,1);
		OnCommand=cmd(sleep,SleepOffset+0.2;linear,0.05;x,SCREEN_RIGHT-OffsetX+105);
...
if GAMESTATE:IsPlayerEnabled(PLAYER_1) then
	t[#t+1]=LoadFont("_compacta blk bt 14px") .. {
		...
		InitCommand=cmd(maxwidth,240;zoomx,1.8;zoomy,1.3;x,SCREEN_LEFT-OffsetX-123;y,SCREEN_CENTER_Y+158;horizalign,center;diffuse,CustomDifficultyToColor( ToEnumShortString(GAMESTATE:GetCurrentSteps(PLAYER_1):GetDifficulty()) ));
		OnCommand=function(self)
			(cmd(sleep,SleepOffset+0.2;linear,0.05;x,SCREEN_LEFT+OffsetX-90))(self);
		end;
	};
end;
...
if GAMESTATE:IsPlayerEnabled(PLAYER_2) then
	t[#t+1]=LoadFont("_compacta blk bt 14px") .. {
		...
		InitCommand=cmd(maxwidth,240;zoomx,1.8;zoomy,1.3;x,SCREEN_RIGHT+OffsetX+123;y,SCREEN_CENTER_Y+158;horizalign,center;diffuse,CustomDifficultyToColor( ToEnumShortString(GAMESTATE:GetCurrentSteps(PLAYER_2):GetDifficulty()) ));
		OnCommand=function(self)
			(cmd(sleep,SleepOffset+0.2;linear,0.05;x,SCREEN_RIGHT-OffsetX+95))(self);
		end;
	};
end;

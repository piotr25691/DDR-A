...
local function PaseliText_P1()
	...
	local text = LoadFont("_futura lt medium 24px") .. {
		InitCommand = cmd(x,SCREEN_CENTER_X-632;y,SCREEN_BOTTOM-10.5;zoom,0.62;strokecolor,color("0.3,0.3,0.3,1");playcommand,"Refresh");
...
local function PaseliText_P2()
	--Text of "EXTRA PASELI : XX"
	local text = LoadFont("_futura lt medium 24px") .. {
		InitCommand = cmd(x,SCREEN_CENTER_X+630;y,SCREEN_BOTTOM-10.5;zoom,0.62;strokecolor,color("0.3,0.3,0.3,1");playcommand,"Refresh");
		RefreshCommand = function(self)
...
local function CreditsText()
	local text = LoadFont("_futura lt medium 24px") .. {
		InitCommand=cmd(x,SCREEN_CENTER_X;y,SCREEN_BOTTOM-10.5;zoom,0.64;strokecolor,color("0.3,0.3,0.3,1");playcommand,"Refresh");
		RefreshCommand=function(self)
		--Other coin modes
...
local function CoinsText()
	local text = LoadFont("_futura lt medium 24px") .. {
		InitCommand=cmd(x,SCREEN_CENTER_X+281;y,SCREEN_BOTTOM-10.5;zoom,0.62;strokecolor,color("0.3,0.3,0.3,1");horizalign,center;playcommand,"Refresh");
		RefreshCommand=function(self)
			local coins=GAMESTATE:GetCoins()
			local coinsPerCredit=PREFSMAN:GetPreference('CoinsPerCredit')
			local remainder=math.mod(coins,coinsPerCredit)
